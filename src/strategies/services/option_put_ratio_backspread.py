...
__all__ = [
    ...
    "OptionPutRatioBackspreadDependency",
...
# -------------------------------------------------
class OptionPutRatioBackspreadService(BaseStrategy):
    def __init__(
        self,
    ...
    async def evaluate(
        ...
        """ """
        if df.empty:
            logger.warning("[OptionPutRatioBackspread] DataFrame is empty")
            return pd.DataFrame()
        ...
        try:
            ...
            df_x1 = df.loc[(df["cficode"].isin([CFICode.put_accion.value]))]
            filter_cols = [
                "ticker",
                "strike",
                "cficode",
            ...
            if not df.empty:
                ...
                df["prime_relation"] = df["adj_prima_x1"] / df["adj_prima_x0"]
                ...
                df["prima_neta"] = (df["adj_prima_x0"] * df["q_compra"]) - (
                    df["adj_prima_x1"] * df["q_venta"]
                )
                ...
                df["pe_inf"] = (
                    df["strike_x0"]
                    - (
                        (df["spread"] * df["q_venta"] * 100)
                        / (df["q_compra"] * 100 - df["q_venta"] * 100)
                    )
                    + df["prima_neta"]
                )
                df["var_pe_inf"] = df["pe_inf"] / df["underlying_close"] - 1
                df["pe_sup"] = df["strike_x1"] + (df["prima_neta"] / df["q_venta"])
                ...
                df["pe_max_loss"] = (df["max_loss"] / 100) + df["pe_inf"]
                df["var_pe_max_loss"] = df["pe_max_loss"] / df["underlying_close"] - 1

                df = df[self.summary_cols]
                df = df.loc[df["spread_pe_sup_inf"] < 0.4]
                df = df.sort_values(by="var_pe_inf", ascending=False)

                async with self.lock:
                    self.summary_strategy_df = df.copy()

        except Exception as e:
            logger.error(f"[OptionPutRatioBackspread] Error en evaluación: {e}")


OptionPutRatioBackspreadDependency = Annotated[
    OptionPutRatioBackspreadService, Depends()
]
OPTION_PUT_RATIO_BACKSPREAD_NAME = "option_put_ratio_backspread"
