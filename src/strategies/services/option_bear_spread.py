...
# -------------------------------------------------
class OptionBearSpreadService(BaseStrategy):
    def __init__(
        self,
        ...
        self._sheet_name = "bear_spread_new"
    ...
    async def evaluate(
        ...
        """ """
        if df.empty:
            logger.warning("[OptionBearSpread] DataFrame is empty")
            return pd.DataFrame()
        ...
        try:
            ...
            if not df.empty:
                ...
                df["prima_neta"] = df["adj_prima_x0"] - df["adj_prima_x1"]
                df = df.loc[
                    ((df["type"] == CFICode.call_accion.name) & (df["prima_neta"] > 0))
                    | (df["type"] == CFICode.put_accion.name)
                ]
                ...
                df["max_profit"] = (
                    np.where(
                        df["type"] == CFICode.put_accion.name,
                        (df["spread"]) + df["prima_neta"],
                        df["prima_neta"],
                    )
                    * 100
                )
                df["max_profit_pct"] = df["max_profit"] / df["min_invest"]
                df["var_max_profit"] = df["strike_x0"] / df["underlying_close"] - 1
                ...
                df["var_max_loss"] = df["strike_x1"] / df["underlying_close"] - 1
                ...
                ]  # Solo aquellas Bears que requieran hasta un 40% de baja
            ...
            logger.error(f"[OptionBearSpread] Error en evaluación: {e}")


OptionBearSpreadDependency = Annotated[OptionBearSpreadService, Depends()]
OPTION_BEAR_SPREAD_NAME = "option_bear_spread"
