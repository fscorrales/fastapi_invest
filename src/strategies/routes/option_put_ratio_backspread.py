# src/stretegies/routes/option_put_ratio_backspread.py
...
from ..schemas import OptionRatioBackspreadFilter, OptionRatioBackspreadSummary
...
STRATEGY_NAME = OPTION_PUT_RATIO_BACKSPREAD_NAME

option_put_ratio_backspread_router = APIRouter(
    prefix="/" + STRATEGY_NAME, tags=["Strategies - Option Put Ratio Backspread"]
)


@option_put_ratio_backspread_router.post("/start", response_model=dict)
async def start_option_put_ratio_backspread(
    ...
    strategy = OptionPutRatioBackspreadService(
...
@option_put_ratio_backspread_router.get("/status", response_model=dict)
...
@option_put_ratio_backspread_router.post("/stop")
async def stop_option_put_ratio_backspread():
    await strategy_manager.stop_strategy(STRATEGY_NAME)
    return {"status": "stopped", "strategy": STRATEGY_NAME}


@option_put_ratio_backspread_router.post("/reset", response_model=dict)
...
@option_put_ratio_backspread_router.get(
    "/dataframe", response_model=list[OptionRatioBackspreadSummary]
)
async def get_strategy_data(
    params: Annotated[OptionRatioBackspreadFilter, Depends()],
):
    strategy = strategy_manager.get(STRATEGY_NAME)
    if not strategy:
        raise HTTPException(status_code=404, detail="La estrategia no está activa")
    ...
    return [OptionRatioBackspreadSummary(**row.to_dict()) for _, row in df.iterrows()]
