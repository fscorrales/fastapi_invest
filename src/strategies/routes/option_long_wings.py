# src/stretegies/routes/option_long_wings.py
...
from ..schemas import OptionLongWingsFilter, OptionLongWingsSummary
...
option_long_wings_router = APIRouter(
    prefix="/" + STRATEGY_NAME,
    tags=["Strategies - Option Longs Wings (Butterfly & Condor)"],
)
...
@option_long_wings_router.get("/dataframe", response_model=list[OptionLongWingsSummary])
async def get_strategy_data(
    ...
    return [OptionLongWingsSummary(**row.to_dict()) for _, row in df.iterrows()]
