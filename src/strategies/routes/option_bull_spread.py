# src/stretegies/routes/option_bull_spread.py
...
STRATEGY_NAME = OPTION_BULL_SPREAD_NAME

option_bull_spread_router = APIRouter(
    prefix="/" + STRATEGY_NAME, tags=["Strategies - Option Bull Spread"]
)


@option_bull_spread_router.post("/start", response_model=dict)
async def start_option_bull_spread(
    ...
    strategy = OptionBullSpreadService(
...
@option_bull_spread_router.get("/status", response_model=dict)
...
@option_bull_spread_router.post("/stop")
async def stop_option_bull_spread():
    await strategy_manager.stop_strategy(STRATEGY_NAME)
    return {"status": "stopped", "strategy": STRATEGY_NAME}


@option_bull_spread_router.post("/reset", response_model=dict)
