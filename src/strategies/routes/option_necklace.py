# src/stretegies/routes/option_necklace.py
...
from ..schemas import OptionNecklaceFilter, OptionNecklaceSummary
...
STRATEGY_NAME = OPTION_NECKLACE_NAME

option_necklace_router = APIRouter(
    prefix="/" + STRATEGY_NAME, tags=["Strategies - Option Necklace"]
)


@option_necklace_router.post("/start", response_model=dict)
async def start_option_necklace(
    ...
    strategy = OptionNecklaceService(
...
@option_necklace_router.get("/status", response_model=dict)
...
@option_necklace_router.post("/reset", response_model=dict)
...
@option_necklace_router.get("/dataframe", response_model=list[OptionNecklaceSummary])
async def get_strategy_data(
    params: Annotated[OptionNecklaceFilter, Depends()],
):
    strategy = strategies_manager.get(STRATEGY_NAME)
    if not strategy:
        raise HTTPException(status_code=404, detail="La estrategia no está activa")
    ...
    return [OptionNecklaceSummary(**row.to_dict()) for _, row in df.iterrows()]
